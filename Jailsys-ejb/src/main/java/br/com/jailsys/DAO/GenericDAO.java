...
public abstract class GenericDAO<T extends EntidadeComum> {
	...
	public List<T> listarPor(String atributo, Object valor) {
		String sql = String.format(QUERY_LISTAR_POR, getTypeClass()
				.getSimpleName(), atributo);
	...
	public List<T> listarPorLike(List<String> atributos, Object valor) {
		...
		Query query = getEntityManager().createQuery(
				"FROM " + getTypeClass().getSimpleName() + queryStr + " AND o.ativo = true");
		return query.getResultList();
	}
	...
	protected EntityManager getEntityManager() {
		if (manager == null) {
			throw new IllegalStateException(
					"getEntityManager() não foi setado antes do uso neste DAO");
		}
		return manager;
	}
