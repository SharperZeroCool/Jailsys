...
public class AtividadeDAO extends GenericDAO<Atividade> implements Serializable {
	...
	public List<Atividade> listarDesvinculadas(String idAmbiente) {
		return getEntityManager().createQuery(
				"SELECT a FROM Atividade a WHERE ((a.ativo = true) AND (a.id NOT IN(SELECT a2.id FROM Ambiente amb2 JOIN amb2.atividades a2 WHERE amb2.id ="+ idAmbiente +")))"
				).getResultList();
	}
	
	public List<Atividade> listarVinculadas(String idAmbiente) {
		return getEntityManager().createQuery(
				"SELECT a FROM Ambiente amb JOIN amb.atividades a WHERE amb.id ="+ idAmbiente +")"
				).getResultList();
	}
